...
void setup () {
  ...
  y = 300;
  //space images
  png = loadImage("fireballpic.png");
  png2 = loadImage("fireball2.png");
  gif = loadImage("rainbowstar.gif");
  Uranus = loadImage ("Uranus.png");
  Saturn = loadImage ("Saturn.png");
  Spacemonster2 = loadImage("Spacemonster 2.png");
  Spacemonster = loadImage ("Spacemonster.png");
...
void draw() {
  ...
  if (abs(mouseX - x) < hr &&
    ...
    image(Spacemonster2,x,y,200,210); 
  //ball 
  tint(255,255);
  stroke(strokecolour);
  strokeWeight (6); 
  ...
  if ( mousePressed == true) {
    ...
    //insert tickle monster 2
    tint(255,200);
  image(Spacemonster,monsterX,monsterY,80,80);
  //Make monster bounce 
  MSpeedY = MSpeedY + 0.1;
  if (monsterY > 200) {
    monsterY = 200; 
  MSpeedY = MSpeedY * -1;}
    else if (monsterY <= 250) {
      monsterY = 250;
    MSpeedY = MSpeedY * -1; } 
    //rainbow road
    noTint();
    ...
    image(png2, BallXi, BallYi, 200, 200);
    //make fireball2 move 
...
void keyPressed () {
  if (key == CODED) {
    if (keyCode == UP) {
      ballcolour = color (98, 152, 239);
      strokecolour = color (249, 193, 109);
    }
